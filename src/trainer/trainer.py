...
class Trainer(BaseTrainer):
    ...
    def log_spectrogram(self, spectrogram, audio, **batch):
        ...
        self.writer.add_audio("audio_with_augmentation", audio[0], SAMPLE_RATE)
    ...
    def log_predictions(
        ...
        beam_search_texts = [self.text_encoder.ctc_beam_search_lm_decode(log_prob[:length,:]) for log_prob, length in zip(log_probs, log_probs_length)]
        ...
        for pred, target, raw_pred, bs_pred, audio_path in tuples[:examples_to_log]:
            ...
            rows[Path(audio_path).name] = {
                ...
                "argmax wer": wer,
                "argmax cer": cer,
                "beam search wer": bs_wer,
                "beam search cer": bs_cer,
            }
